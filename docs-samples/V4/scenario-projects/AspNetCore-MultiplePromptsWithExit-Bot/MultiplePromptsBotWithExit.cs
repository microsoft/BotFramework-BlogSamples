...
namespace AspNetCore_Multiple_Prompts_With_Exit
{
    ...
    public class MultiplePromptsBotWithExit : IBot
    {
        ...
        private async Task NameValidator(ITurnContext context, TextResult result)
        {
            ...
            // Check if user entered an exit phrase.
            if ((result.Text.ToLower() == "stop") || (result.Text.ToLower() == "cancel") || (result.Text.ToLower() == "quit") || (result.Text.ToLower() == "exit"))
            {
                await context.SendActivity("Goodbye!");
                // Flag current session as over.
                sessionOver = true;
            }
        }
        ...
        {
            // Check if user entered an exit value.
            if (result.Value <= 0)
            {
                await context.SendActivity("Goodbye!");
                // Flag current session as over.
        ...
        {
            await dialogContext.Prompt(PromptStep.NamePrompt, "What is your name? (enter 'stop' to exit)");
        }
        ...
        private async Task AskAgeStep(DialogContext dialogContext, object result, SkipStepFunction next)
        {
            ...
            { 
                ...
                await dialogContext.Prompt(PromptStep.AgePrompt, "What is your age? (enter 0 or -1 to exit)");
            }
            else
            {
        ...
        public MultiplePromptsBotWithExit()
        {
